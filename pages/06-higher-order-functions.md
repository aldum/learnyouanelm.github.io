...
map ((+) 3) [1,5,3,1,6]
[4,8,6,4,9]
map (flip (++) "!") ["BIFF", "BANG", "POW"]
["BIFF!","BANG!","POW!"]
map (List.repeat 3) [3, 4, 5, 6]
[[3,3,3],[4,4,4],[5,5,5],[6,6,6]]
map (map (flip (^) 2)) [[1,2],[3,4,5,6],[7,8]]
[[1,4],[9,16,25,36],[49,64]]
map Tuple.first [(1,2),(3,5),(6,3),(2,6),(2,5)]
[1,3,6,2,2]
```
...
filter ((==) 3) [1,2,3,4,5]
[3]
filter (let nonEmpty ls = not (List.isEmpty ls) in nonEmpty) [[1,2,3],[],[3,4,5],[2,2],[],[],[]]
[[1,2,3],[3,4,5],[2,2]]
```
...
largestDivisible =
    let
        p x = x % 3829 == 0
    in
    List.head (filter p List. reverse (List.range 0 99999))
```

We first make a list of all numbers between lower than 100,000, descending. Then
...
let
    odd n = n % 2 /= 0
in
List.sum (takeWhile (flip (<) 10000) (List.filter odd (List.map (flip (^) 2) (List.range 0 10000))))
166650
```
...
chain n = 
    ...
    case n of
        1 -> [1]
        n -> if even n then n :: chain (n // 2) else n :: chain (n * 3 + 1)
```
...
numLongChains : Int
numLongChains = 
  let isLong xs = List.length xs > 15
  in
...
Using map, we can also do stuff like List.map (\*) (List.range 0 100), if not for any
...
like List.map ((\*) 2) (List.range 1 100) to get a list of type List number, but we can
also do List.map (\*) (List.range 1 100) without a problem. What happens here is that the
...
If we map \* over the list List.range 0 100, we get back a list of functions that
...
If you look about 5 inches up, you'll see that we used a *let* binding
...
```elm
numLongChains : Int
numLongChains = List.length (List.filter (\xs -> List.length xs > 15) (List.map chain (List.range 1 100)))
```
...
It would be map f xs = foldl (\\x acc -\> acc ++ [f x]) [] xs, but the
...
accumulator. In the end, we build up a reversed list. \\x acc -\> x ::
...
scanl (currently there is no scanr in the standard library) is like foldl,
only it reports all the intermediate accumulator states in the form of a list.
...
How about List.sum (List.filter ((\>) 10) (List.map ((\*) 2) (List.range 2 10)))? Well, because <| is
...
```elm
List.map (\xs -> negate (List.sum (List.take 2 xs))) [List.range 1 5,List.range 3 6,List.range 1 7]
[-3,-7,-3]
```
...
```elm
List.map (negate << List.sum << List.take 2) [List.range 1 5,List.range 3 6,List.range 1 7] 
[-3,-7,-3]
```
...
have List.repeat 100 (List.product (List.map ((*) 3) (List.map2 max [1,2,3,4,5] [4,5,6,7,8]))),
...
oddSquareSum = 
    let
        odd n = n % 2 == 1
    in
    List.sum (takeWhile (flip (<) 10000) (List.filter odd (List.map (flip (^) 2) (List.range 1 9999))))
```
